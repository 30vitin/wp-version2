<?php 
	...
	$class = 'grid-post '.esc_attr(fashow_get_class()->class_item_blog);
?>
<article id="post-<?php esc_attr(the_ID()); ?>" <?php post_class($class); ?>>
	<div class="entry-post">
		...
		<?php elseif( $format == 'video' || $format == 'audio' ) : ?>
			<div class="entry-thumb single-thumb">
				<a class="post-thumbnail" href="<?php esc_url(the_permalink()); ?>" title="<?php echo the_title_attribute(); ?>">
					<?php the_post_thumbnail( 'fashow-full-width', array( 'alt' => get_the_title() ) );	?>      
				</a> 
			</div>	
						...
						<div class="slick-carousel" data-columns4="1" data-columns3="1" data-columns2="1" data-columns1="1" data-columns="1" data-nav="true">
								<?php
									if($ids){
										...
										foreach ( $ids as $i => $id ){ ?>
											<div class="item">			
													<?php echo wp_get_attachment_image($id, 'fashow-full-width'); ?>
											</div>
										<?php }	
									}
								?>
		...
		<div class="post-content">
        	...
        	<?php
				fashow_posted_on();
        		if (fashow_get_config('blog-excerpt')) {
                    echo fashow_get_excerpt( fashow_get_config('grid-blog-excerpt-length',30), true);
                }
			?> 
		</div>
	</div>
